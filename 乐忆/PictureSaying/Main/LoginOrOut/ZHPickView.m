...
-(NSArray *)getPlistArrayByplistName:(NSString *)plistName{
    ...
    for (NSDictionary *dic in array) {
        NSString *shengfen = [[dic allKeys] objectAtIndex:0];
        [shengArray addObject:shengfen];
...
-(void)setToolbarWithPickViewFrame{
    ...
    leftButton.frame = CGRectMake(20, 0, 50, _toolbar.height);
    ...
    rightButton.frame = CGRectMake(KScreenWidth-70, 0, 50, _toolbar.height);
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    if (component == 0) {
//        city = [[_plistArray[component] allKeys] objectAtIndex:0];
        shengfen = shengArray[row];
        NSInteger ind = [shengArray indexOfObject:shengfen];
        city = [cityArray[ind] objectAtIndex:0];
    }else{
        
        NSInteger ind = [shengArray indexOfObject:shengfen];
        city = [cityArray[ind] objectAtIndex:row];
    }
    
//    _resultString = city;
//    NSLog(@"cittty === %@",_plistArray);
    ...
    _resultString = [NSString stringWithFormat:@"%@-%@",shengfen,city];
}
